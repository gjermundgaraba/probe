...
HTTPS_GIT := github.com/cosmos/probe.git
...
ldflags = -X github.com/cosmos/cosmos-sdk/version.Name=probe \
		  -X github.com/cosmos/cosmos-sdk/version.AppName=probed \
...
ifeq ($(OS),Windows_NT)
	...
	go build -mod=readonly $(BUILD_FLAGS) -o build/probed ./cmd/probed
endif

build-windows-client: go.sum
	GOOS=windows GOARCH=amd64 go build -mod=readonly $(BUILD_FLAGS) -o build/probed.exe ./cmd/probed
...
install: go.sum
	go install -mod=readonly $(BUILD_FLAGS) ./cmd/probed
...
draw-deps:
	...
	@goviz -i ./cmd/probed -d 2 | dot -Tpng -o dependency-graph.png
